...
def format_output(I,M,U):
    ...
    with open('result_als.txt','w') as f:
        for i in range(250): # users
            for j in range(10000): # items
...
with open("process_traindata.txt") as f:
    tr = np.loadtxt(f,usecols=range(0,3),dtype=np.int,delimiter=',')
with open("process_validata.txt") as f:
    tst = np.loadtxt(f,usecols=range(0,3),dtype=np.int,delimiter=',')

R = np.asmatrix(np.zeros((num_users,num_items), dtype=np.int)) # sparse rating matrix
for row in tr:
...
Rtst = np.asmatrix(np.zeros((num_users,num_items), dtype=np.int))
for row in tst:
    user = row[0] - 1
    movie = row[1] - 1
    rating = row[2]
...
avg_items = np.true_divide(R.sum(0),(R!=0).sum(0))
avg_items[np.isnan(avg_items)] = 0
M[0,:] = avg_items # Set first row of Q to column vector of average ratings
...
while k < K:
    ...
    for i, Ii in enumerate(I):
        ...
        Ai = np.add(np.dot(M, np.dot(np.diag(Ii), M.T)),
                    np.multiply(l, np.multiply(nui, E)))  # A_i = M_{I_i}M_{I_i}^T + ln_{u_i}E
    ...
    for j, Ij in enumerate(I.T):
        ...
        # Least squares solution
        Aj = np.add(np.dot(U, np.dot(np.diag(Ij), U.T)),
                    np.multiply(l, np.multiply(nmj, E)))  # A_j = U_{I_j}U_{I_j}^T + ln_{m_j}E
        Vj = np.dot(U, np.dot(np.diag(Ij), R[:, j]))  # V_j = U_{I_j}R(I_j,j)
        try:
            M[:, j] = np.linalg.solve(Aj, Vj).reshape(r)
        except:
            print
            "singular matrix error: " + str(j + 1)  # movie has no reviewers
            continue
